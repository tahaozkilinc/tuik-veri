...
indirip Mısır / Soya Fasulyesi / Soya Küspesi bilanço tablolarını
ayrıştırır — Supabase'e hiçbir şey yazmaz, sadece keşif/doğrulama amaçlı.
...
REGIONS = [
    ("World", "World", "World"),
    ("World Less China", "  World Less China", "  World Less China"),
...
def find_latest_txt_url() -> str:
    html = fetch(PUB_URL)
    href_re = re.compile(r'<a[^>]*href="([^"]+\.txt)"[^>]*>', re.I)
    matches = href_re.findall(html)
    if not matches:
        raise RuntimeError("WASDE yayın sayfasında .txt linki bulunamadı")
    txt_url = matches[0]
...
def parse_world_table(block: str, ncols: int) -> dict[str, dict[str, list[float]]]:
    ...
    out: dict[str, dict[str, list[float]]] = {}
    for name, lab_flat, lab_proj in REGIONS:
...
def main() -> None:
    txt_url = find_latest_txt_url()
    print(f"En son WASDE TXT: {txt_url}", file=sys.stderr)
    text = fetch(txt_url)
    print(f"txt uzunluğu: {len(text)}", file=sys.stderr)

    print("\n=== U.S. CORN (Million Bushels, fiyat $/bu) ===")
    for label, vals in parse_us_corn(text).items():
        print(f"{label:35s} {vals}")

    print("\n=== U.S. SOYBEANS / OIL / MEAL ===")
    for sub, rows in parse_us_soy(text).items():
        print(f"--- {sub} ---")
        for label, vals in rows.items():
            print(f"{label:35s} {vals}")

    print("\n=== WORLD CORN (Million Metric Tons: BegStocks,Prod,Imports,Feed,DomTotal,Exports,EndStocks) ===")
    for region, series in parse_world_corn(text).items():
        print(f"-- {region} --")
    ...
    for region, series in parse_world_soy(text).items():
        print(f"-- {region} --")
        for period, vals in series.items():
            print(f"  {period:20s} {vals}")

    print("\n=== WORLD SOYBEAN MEAL (Million Metric Tons: BegStocks,Prod,Imports,DomTotal,Exports,EndStocks) ===")
    for region, series in parse_world_soymeal(text).items():
        print(f"-- {region} --")
        for period, vals in series.items():
            print(f"  {period:20s} {vals}")
